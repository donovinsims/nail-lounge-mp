...
import { z } from "zod";
import { fetchSalon, fetchServices, fetchStaff, computeAvailableSlots, fmtMoney, fmtTime, fmtDate } from "@/lib/salon";
...
export const Route = createFileRoute("/book")({
  ...
  head: () => ({ meta: [{ title: "Book — Nail Lounge" }, { name: "description", content: "Pick a service, technician, and time." }] }),
  component: Book,
});
...
function Book() {
  ...
  const { data: services = [] } = useQuery({
    queryKey: ["services", salon?.id], queryFn: () => fetchServices(salon!.id), enabled: !!salon,
  });
  ...
  useEffect(() => {
    if (search.service && services.some((s) => s.id === search.service)) setServiceId(search.service);
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-3xl px-6 py-20 text-center sm:py-28">
          ...
          <p className="mt-6 text-base text-muted-foreground">
            Four short steps — service, artist, time, details. We'll hold your spot with a small deposit.
          </p>
      ...
      <BottomSheet
        ...
        title={
          <div className="flex items-center gap-3">
            {step > 1 && <button onClick={back} className="-ml-2 p-2"><ChevronLeft className="h-5 w-5" /></button>}
          ...
          step === 4 ? (
            <button
              ...
              onClick={() => mutation.mutate({ data: {
                salonId: salon!.id, serviceId: service!.id, staffId: tech!.id,
                startTime: slot!.toISOString(), clientName: name, clientPhone: phone, clientEmail: email,
          ...
          services.length === 0 ? (
            ...
            <ul className="space-y-2">
              {services.map((s) => (
                <li key={s.id}>
                  <button
                    onClick={() => { setServiceId(s.id); setTimeout(next, 150); }}
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">{s.category} · {s.duration_minutes} min</p>
                    </div>
                    <p className="font-mono text-sm font-semibold shrink-0">{fmtMoney(Number(s.price))}</p>
                  </button>
                </li>
              ))}
            </ul>
        ...
        {step === 2 && (
          <ul className="space-y-2">
            {staff.map((t: any) => (
              <li key={t.id}>
                <button
                  onClick={() => { setStaffId(t.id); setTimeout(next, 150); }}
                  className={`flex w-full tap-target items-center gap-4 rounded-2xl bg-surface p-4 text-left ${staffId === t.id ? "ring-2 ring-ring" : ""}`}
                >
                  <div className="h-11 w-11 shrink-0 rounded-full grid place-items-center text-white font-semibold" style={{ background: t.avatar_color || "#000" }}>
                    {t.name[0]}
                  </div>
                  <div className="min-w-0">
          ...
          <div>
            <div className="-mx-1 flex gap-2 overflow-x-auto pb-3">
              {Array.from({ length: 14 }, (_, i) => {
                ...
                return (
                  <button key={i} onClick={() => { setDate(d); setSlot(null); }}
            ...
            <div className="mt-4">
              {loadingSlots ? (
                ...
                <p className="py-10 text-center text-sm text-muted-foreground">No availability this day. Try another.</p>
              ) : (
        ...
        {step === 4 && (
          <div className="space-y-4">
            <div className="rounded-2xl bg-surface p-4 text-sm space-y-1">
              ...
              <div className="flex justify-between"><span className="text-muted-foreground">With</span><span className="font-medium">{tech?.name}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">When</span><span className="font-medium">{slot && `${fmtDate(slot)}, ${fmtTime(slot)}`}</span></div>
              <div className="mt-2 flex justify-between border-t pt-2"><span className="text-muted-foreground">Total</span><span className="font-mono font-semibold">{fmtMoney(Number(service?.price ?? 0))}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Deposit today</span><span className="font-mono font-semibold">{fmtMoney(Number(service?.deposit_amount ?? 0))}</span></div>
            </div>
            <div className="space-y-2">
              <Field label="Full name" value={name} onChange={setName} placeholder="Jane Doe" />
              <Field label="Phone" value={phone} onChange={setPhone} type="tel" placeholder="(815) 555-0123" />
            ...
            <div className="rounded-2xl bg-surface p-4">
              ...
              <p className="mt-2 text-sm">Tap card on file <span className="font-mono">•••• 4242</span></p>
            </div>
          </div>
        )}
      </BottomSheet>
