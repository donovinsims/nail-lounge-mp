...
function BookingConfirmed() {
  ...
  const { data, isPending, isError, error } = useQuery({
    ...
    queryFn: () =>
      (
        fetchDetails as (input: { bookingId: string }) => Promise<{
          id: string;
          startTime: string;
          endTime: string;
          status: string;
  ...
  return (
    <div className="flex min-h-screen flex-col bg-background text-foreground">
      ...
      {(isError && !isNetworkError) || (!data && !isPending && !isError && !isNetworkError) ? (
        <div className="flex flex-1 items-center justify-center px-4 py-32">
          <div className="w-full max-w-md rounded-2xl bg-surface p-8 text-center shadow-lg space-y-5">
            ...
            <div className="rounded-2xl bg-surface p-6 shadow-lg space-y-4">
              ...
              <div className="border-t border-border pt-4">
                <div className="rounded-xl bg-warning/15 p-4 text-sm text-warning-ink space-y-1">
                  ...
                  <p>
                    Pay in-studio at the time of your appointment. We accept credit/debit, cash,
                    Venmo, and Cash App.
                  </p>
                </div>
              </div>
