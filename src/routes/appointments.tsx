...
function Appointments() {
  ...
  const cancelMutation = useMutation({
    mutationFn: cancel,
    onSuccess: () => { toast.success("Cancelled"); lookupMutation.mutate({ data: { phone } }); },
  ...
  return (
    <div className="min-h-screen bg-background px-5 py-6 safe-pt">
      ...
      <form onSubmit={(e) => { e.preventDefault(); lookupMutation.mutate({ data: { phone } }); }} className="mt-6 flex gap-2">
        <input value={phone} onChange={(e) => setPhone(e.target.value)} type="tel" placeholder="(815) 555-0123"
          className="flex-1 tap-target rounded-xl bg-surface px-4 outline-none focus:ring-2 focus:ring-ring" />
        <button className="tap-target rounded-xl bg-primary px-5 font-medium text-primary-foreground" disabled={lookupMutation.isPending}>
          {lookupMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Find"}
        </button>
      </form>

      <ul className="mt-8 space-y-3">
        {lookupMutation.data?.length === 0 && <p className="text-sm text-muted-foreground">No appointments found.</p>}
        {lookupMutation.data?.map((b: any) => (
          <li key={b.id} className="rounded-2xl bg-surface p-4">
            <div className="flex items-baseline justify-between gap-3">
              ...
              <span className={`text-[10px] font-mono uppercase tracking-wider px-2 py-1 rounded-full ${b.status === 'cancelled' ? 'bg-destructive/10 text-destructive' : b.status === 'completed' ? 'bg-success/10 text-success' : 'bg-surface-2'}`}>{b.status}</span>
            </div>
            <p className="mt-2 text-sm">{fmtDate(b.start_time)} · {fmtTime(b.start_time)}</p>
            <p className="mt-1 text-xs text-muted-foreground">{fmtMoney(Number(b.services?.price ?? 0))} · deposit {fmtMoney(Number(b.deposit_paid))}</p>
            {b.status === "confirmed" && (
              <button
