  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      >
        ...
        <button
          onClick={signOut}
          className="flex items-center gap-3 px-5 py-4 text-sm text-muted-foreground hover:text-foreground border-t border-sidebar-border"
        >
      ...
      <nav className="md:hidden fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-border bg-background/95 backdrop-blur safe-pb">
        {NAV.slice(0, 4).map((n) => (
          <button
            key={n.id}
            onClick={() => setTab(n.id)}
            className={`flex flex-col items-center gap-1 py-2 text-[10px] ${tab === n.id ? "text-foreground" : "text-muted-foreground"}`}
          >
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <Stat
          label="Confirmed"
          value={String(bookings.filter((b: any) => b.status === "confirmed").length)}
        />
      </div>
      <div className="rounded-2xl bg-surface">
        ...
        <ul className="divide-y divide-border">
          {bookings.length === 0 && (
            <li className="p-5 text-sm text-muted-foreground">No bookings today.</li>
          )}
          {bookings.map((b: any) => (
            <li key={b.id} className="grid grid-cols-[80px_1fr_auto] items-center gap-3 p-5">
              ...
              <div className="min-w-0">
                <p className="truncate font-medium">
                  {b.services?.name} · {b.clients?.name}
                </p>
                <p className="text-xs text-muted-foreground">with {b.staff?.name}</p>
              </div>
              <span
                className={`text-[10px] font-mono uppercase tracking-wider px-2 py-1 rounded-full ${b.status === "completed" ? "bg-success/10 text-success" : "bg-surface-2"}`}
              >
                {b.status}
              </span>
            </li>
  ...
  const { data: bookings = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("bookings")
        .select(
          "id, start_time, end_time, status, services(name), staff(name, avatar_color), clients(name)",
        )
        .eq("salon_id", salonId)
        .gte("start_time", start.toISOString())
        .lte("start_time", end.toISOString())
        ...
        {Array.from({ length: 14 }, (_, i) => {
          ...
          return (
            <button
              key={i}
              onClick={() => setDate(d)}
              className={`shrink-0 flex flex-col items-center rounded-2xl px-4 py-3 ${sel ? "bg-primary text-primary-foreground" : "bg-surface"}`}
            >
      ...
      <div className="rounded-2xl bg-surface overflow-hidden">
        {Array.from({ length: 12 }, (_, i) => {
          ...
          return (
            <div
              key={hour}
              className="grid grid-cols-[80px_1fr] border-b border-border last:border-0 min-h-[64px]"
            >
              ...
              <div className="p-2 space-y-1">
                {items.map((b: any) => (
                  <div
                    ...
                    style={{ background: (b.staff?.avatar_color || "#000") + "22" }}
                  >
                    <p className="font-semibold truncate">
                      {fmtTime(b.start_time)} · {b.services?.name}
                    </p>
                    <p className="text-muted-foreground truncate">
                      {b.clients?.name} · {b.staff?.name}
                    </p>
                  </div>
                ))}
              </div>
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {floor.map((f: any) => (
        <button
          ...
          className="rounded-2xl bg-surface p-5 text-left active:scale-[0.99]"
        >
          <div className="flex items-center gap-3">
            <div
              className="h-12 w-12 shrink-0 rounded-full grid place-items-center text-white font-semibold"
              style={{ background: f.staff?.avatar_color }}
            >
              {f.staff?.name?.[0]}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <span
                className={`mt-1 inline-block text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded-full ${color(f.status)}`}
              >
                {f.status.replace("_", " ")}
              </span>
            </div>
      ...
      <p className="text-xs text-muted-foreground col-span-full">
        Tap to cycle status · live updates via realtime
      </p>
    </div>
  );
}
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <div className="rounded-2xl bg-surface">
        ...
        <ul className="divide-y divide-border">
          {bookings.length === 0 && (
            <li className="p-5 text-sm text-muted-foreground">No open tickets.</li>
          )}
          {bookings.map((b: any) => (
            <li key={b.id}>
              <button
                ...
                className={`w-full grid grid-cols-[1fr_auto] items-center gap-3 p-4 text-left hover:bg-surface-2 ${sel?.id === b.id ? "bg-surface-2" : ""}`}
              >
                <div className="min-w-0">
                  <p className="truncate font-medium">
                    {b.clients?.name} · {b.services?.name}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {b.staff?.name} · {fmtTime(b.start_time)}
                  </p>
                </div>
      ...
      <div className="rounded-2xl bg-surface p-5">
        {!sel ? (
          <p className="text-sm text-muted-foreground">Select a ticket to take payment.</p>
        ) : (
          <div className="space-y-5">
            ...
            <div className="flex items-baseline justify-between border-b border-border pb-3">
              ...
              <span className="font-mono text-lg">{fmtMoney(Number(sel.services?.price))}</span>
            </div>
            <div>
              ...
              <div className="mt-2 flex gap-2">
                {[0, 5, 10, 15, 20].map((t) => (
                  <button
                    key={t}
                    onClick={() => setTip(t)}
                    className={`tap-target flex-1 rounded-lg text-sm ${tip === t ? "bg-primary text-primary-foreground" : "bg-surface-2"}`}
                  >
                    ${t}
                  </button>
                ))}
            ...
            <div>
              <label className="text-xs font-mono uppercase tracking-widest text-muted-foreground">
                Tip split: {tipSplit}% tech / {100 - tipSplit}% salon
              </label>
              <input
                type="range"
                min={0}
            ...
            <div className="flex items-baseline justify-between text-lg font-semibold">
              <span>Total</span>
              <span className="font-mono">{fmtMoney(Number(sel.services?.price) + tip)}</span>
            </div>
            <button
              disabled={paying}
              onClick={checkout}
              className="flex w-full tap-target items-center justify-center gap-2 rounded-xl bg-primary py-4 font-semibold text-primary-foreground disabled:opacity-50"
            >
              {paying ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" /> Tap card on terminal...
                </>
              ) : (
                <>
...
function Commissions({ salonId }: { salonId: string }) {
  ...
  const total = rows.reduce(
    (a: number, r: any) => a + Number(r.tech_share) + Number(r.tip_to_tech),
    0,
  );
    ...
    <div>
      <div className="grid sm:grid-cols-3 gap-3 mb-6">
        ...
        <Stat
          ...
          value={fmtMoney(
            rows.reduce(
              (a: number, r: any) => a + Number(r.salon_share) + Number(r.tip_to_salon),
              0,
            ),
          )}
        />
      ...
      <div className="flex justify-end mb-3">
        <button
          onClick={exportCsv}
          className="inline-flex items-center gap-2 rounded-lg bg-surface px-4 py-2 text-sm hairline"
        >
            ...
            {rows.length === 0 && (
              <tr>
                <td colSpan={7} className="p-8 text-center text-muted-foreground">
                  No records yet. Complete a POS checkout.
                </td>
              </tr>
            )}
          </tbody>
    ...
    <div className="space-y-4">
      <div className="rounded-2xl bg-surface p-5 flex flex-wrap gap-2">
        <input
          ...
          className="flex-1 min-w-[150px] tap-target rounded-lg bg-surface-2 px-3"
        />
        <input
          placeholder="Phone"
        ...
        <button
          onClick={add}
          className="tap-target rounded-lg bg-primary px-5 text-sm font-medium text-primary-foreground"
        >
          Add
        </button>
      </div>
      <ul className="space-y-2">
        {rows.map((r: any) => (
          <li
            key={r.id}
            className="rounded-2xl bg-surface p-4 flex items-start justify-between gap-3"
          >
            <div className="min-w-0">
              ...
              {r.flagged_booking_id && (
                <p className="mt-2 text-xs font-mono uppercase tracking-wider text-success">
                  ⚡ Slot opened
                </p>
              )}
            </div>
            <span
              className={`text-[10px] font-mono uppercase tracking-wider px-2 py-1 rounded-full ${r.status === "active" ? "bg-surface-2" : "bg-success/10 text-success"}`}
            >
              {r.status}
            </span>
          </li>
        ...
        {rows.length === 0 && (
          <li className="text-sm text-muted-foreground p-4">No waitlist entries.</li>
        )}
      </ul>
    </div>
  );
...
function Calls({ salonId }: { salonId: string }) {
  ...
  return (
    <ul className="space-y-3">
      {rows.length === 0 && (
        <p className="text-sm text-muted-foreground">
          No AI calls yet. Use the Seed demo button on the dashboard.
        </p>
      )}
      {rows.map((c: any) => (
        <li key={c.id} className="rounded-2xl bg-surface p-5">
          ...
          {c.intent === "book_appointment" && !c.converted_booking_id && (
            <button
              ...
              className="mt-3 inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground"
            >
    ...
    <div className="max-w-xl space-y-4">
      <div className="rounded-2xl bg-surface p-5 space-y-3">
        ...
        <label className="block">
          <span className="text-xs text-muted-foreground">Salon name</span>
          <input
            value={name}
            onChange={(e) => setName(e.target.value)}
            className="mt-1 w-full tap-target rounded-lg bg-surface-2 px-3"
          />
        </label>
        <label className="block">
      ...
      <div className="rounded-2xl bg-surface p-5 space-y-3">
        ...
        <label className="block">
          <span className="text-xs text-muted-foreground">Tech commission: {commission}%</span>
          <input
            type="range"
            min={0}
            max={100}
        ...
        <label className="block">
          <span className="text-xs text-muted-foreground">Default tip to tech: {tipSplit}%</span>
          <input
            type="range"
            min={0}
            max={100}
      ...
      <button
        onClick={save}
        className="tap-target rounded-lg bg-primary px-6 font-semibold text-primary-foreground"
      >
        Save changes
      </button>
      <p className="text-xs text-muted-foreground">
        Hours and holiday schedules per salon are stored as JSON in the salon record and can be
        edited here in a future update.
      </p>
    </div>
  );
}
